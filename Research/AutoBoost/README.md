...
Further investigation with WinDbg showed that certain threads from any GUI process are susceptible to boosts into the real-time range, and in fact get boosted during any mouse movement over their window. During mouse movement, threads undergo dozens of back and forth priority switches per second, switching between their original priority and the priority of DWM threads. These boosts also occur to all relevant GUI threads when a change of the foreground window occurs.
